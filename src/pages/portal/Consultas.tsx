...
const formatDateTime = (d: string) => {
  return new Date(d).toLocaleDateString('pt-PT', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' });
};
...
const CALENDLY_URL = 'https://calendly.com/nieusync';
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '32px' }}>
        <div>
          ...
          <p style={{ color: 'rgba(35,56,119,0.60)' }}>
            As suas consultas agendadas e o histórico de reuniões.
          </p>
        </div>
        ...
        {futuras.length === 0 ? (
          <div className="card" style={{ gridColumn: '1 / -1' }}>
            <p style={{ fontSize: '15px', color: 'rgba(35,56,119,0.55)', textAlign: 'center', padding: '20px 0' }}>
              Não tem consultas agendadas. Clique em "Agendar nova consulta" para marcar.
            </p>
          </div>
        ) : (
          futuras.map((c) => (
            <div key={c.id} className="card">
              ...
              <button onClick={() => handleCancelar(c)} className="btn-secondary btn-sm" style={{ borderColor: '#e53e3e', color: '#e53e3e' }}>
                Cancelar consulta
              </button>
            </div>
          ))
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div className="card" style={{ maxWidth: '480px', width: '100%', padding: '32px' }}>
            ...
            <p style={{ fontSize: '14px', color: 'rgba(35,56,119,0.65)', marginBottom: '20px' }}>
              Escolha a data e hora preferidas. A equipa NIEUSYNC confirmará por email.
            </p>
            ...
            <form onSubmit={handleAgendar} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <textarea
                  id="notas"
                  value={novaNotas}
                  onChange={(e) => setNovaNotas(e.target.value)}
                  rows={3}
                  placeholder="Tópicos que gostaria de abordar..."
                  style={{ resize: 'vertical' }}
                />
              </div>
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
                <a href={CALENDLY_URL} target="_blank" rel="noopener noreferrer" style={{ fontSize: '13px', color: 'var(--purple)', textDecoration: 'underline' }}>
                  Ou agendar via Calendly →
                </a>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button type="button" onClick={() => setShowModal(false)} className="btn-secondary btn-sm">
                    Cancelar
                  </button>
                  <button type="submit" className="btn-gradient btn-sm" disabled={saving}>
                    {saving ? 'A agendar...' : 'Confirmar'}
                  </button>
                </div>
              </div>
            </form>
      ...
      <style>{`
        @media (max-width: 768px) {
          .consultas-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  );
