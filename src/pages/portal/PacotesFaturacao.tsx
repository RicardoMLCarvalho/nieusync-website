...
export default function PacotesFaturacao() {
  ...
  const handleDownload = async (fatura: Fatura) => {
    ...
    const { data, error } = await supabase
      ...
      .createSignedUrl(fatura.ficheiro_url, 60);
  ...
  return (
    <div>
      ...
      <p style={{ color: 'rgba(35,56,119,0.60)', marginBottom: '32px' }}>
        Consulte o seu pacote actual, explore pacotes disponíveis e descarregue as suas faturas.
      </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
            <div>
              ...
              <p style={{ fontSize: '14px', color: 'rgba(35,56,119,0.65)' }}>
                {formatDate(pacoteActual.data_inicio)} — {formatDate(pacoteActual.data_fim)}
              </p>
              {pacoteActual.valor !== null && (
                <p style={{ fontSize: '16px', color: 'var(--blue)', fontWeight: 700, marginTop: '8px' }}>
                  {formatValor(pacoteActual.valor)}
                </p>
              )}
            </div>
            <span className={`badge ${pacoteActual.estado === 'ativo' ? 'badge-blue' : pacoteActual.estado === 'expirado' ? 'badge-outline' : 'badge-purple'}`}>
              {pacoteActual.estado === 'ativo' ? 'Ativo' : pacoteActual.estado === 'expirado' ? 'Expirado' : 'Pendente'}
            </span>
          </div>
        ) : (
          <p style={{ fontSize: '15px', color: 'rgba(35,56,119,0.55)' }}>
            Não tem nenhum pacote atribuído. Contacte-nos para saber mais.
          </p>
        )}
      </div>
        ...
        {PACOTES_DISPONIVEIS.map((p) => (
          <div key={p.name} className="card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <ul style={{ listStyle: 'none', marginBottom: '24px', flex: 1, padding: 0 }}>
              {p.includes.map((item) => (
                <li key={item} style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 400, fontSize: '13px', color: 'rgba(35,56,119,0.75)', padding: '5px 0', paddingLeft: '18px', position: 'relative' }}>
            ...
            <div style={{ marginBottom: '16px', textAlign: 'center' }}>
              <span style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 700, fontSize: '16px', color: 'var(--blue)' }}>Sob consulta</span>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-gradient btn-sm" style={{ flex: 1, justifyContent: 'center' }}>
                WhatsApp
              </a>
              <a href={MAILTO_URL} className="btn-secondary btn-sm" style={{ flex: 1, justifyContent: 'center' }}>
                Email
              </a>
            </div>
          </div>
      ...
      <div className="card">
        {faturas.length === 0 ? (
          <p style={{ fontSize: '15px', color: 'rgba(35,56,119,0.55)', textAlign: 'center', padding: '20px 0' }}>
                ...
                {faturas.map((f) => (
                  <tr key={f.id} style={{ borderBottom: '1px solid rgba(159,142,194,0.12)' }}>
                    ...
                    <td style={{ padding: '14px 8px' }}>
                      <span className={`badge ${f.estado === 'paga' ? 'badge-blue' : 'badge-purple'}`}>
                        {f.estado === 'paga' ? 'Paga' : 'Pendente'}
                      </span>
                    </td>
                    <td style={{ padding: '14px 8px', textAlign: 'right' }}>
                      {f.ficheiro_url ? (
                        <button onClick={() => handleDownload(f)} className="btn-secondary btn-sm" style={{ padding: '8px 16px' }}>
                          Descarregar
                        </button>
                      ) : (
                        <span style={{ fontSize: '13px', color: 'rgba(35,56,119,0.40)' }}>—</span>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      <style>{`
        @media (max-width: 1024px) {
          .pacotes-grid { grid-template-columns: repeat(2, 1fr) !important; }
        }
