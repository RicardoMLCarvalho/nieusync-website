...
export default function Documentos() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    if (dbError) {
      setUploadError('Ficheiro enviado, mas houve um erro a registar. Contacte-nos.');
      setUploading(false);
      return;
    }
  ...
  return (
    <div>
      ...
      <p style={{ color: 'rgba(35,56,119,0.60)', marginBottom: '32px' }}>
        Troque documentos connosco e acompanhe o estado de cada ficheiro.
      </p>
      ...
      <div className="card" style={{ marginBottom: '32px' }}>
        ...
        {uploadError && (
          <div style={{ background: 'rgba(229,62,62,0.10)', border: '1px solid rgba(229,62,62,0.30)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px' }}>
            <p style={{ color: '#e53e3e', fontSize: '14px', margin: 0 }}>{uploadError}</p>
          </div>
        )}
        {uploadMsg && (
          <div style={{ background: 'rgba(34,139,87,0.10)', border: '1px solid rgba(34,139,87,0.30)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px' }}>
            <p style={{ color: '#228B57', fontSize: '14px', margin: 0 }}>{uploadMsg}</p>
          </div>
        )}
        ...
        <form onSubmit={handleUpload} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div>
            ...
            <select id="categoria" value={uploadCategoria} onChange={(e) => setUploadCategoria(e.target.value as CategoriaDocumento)}>
          ...
          <div>
            ...
            />
            {uploadFile && (
              <p style={{ fontSize: '13px', color: 'rgba(35,56,119,0.60)', marginTop: '8px' }}>
                Selecionado: {uploadFile.name} ({(uploadFile.size / 1024).toFixed(0)} KB)
              </p>
            )}
          </div>
          <button type="submit" className="btn-gradient" disabled={uploading || !uploadFile} style={{ alignSelf: 'flex-start' }}>
        ...
        {(['Todos', ...CATEGORIAS_DOCUMENTOS] as const).map((cat) => (
          <button
            ...
            style={{
              ...
              borderRadius: '100px', cursor: 'pointer', minHeight: '40px',
      ...
      <div className="card">
        {filtered.length === 0 ? (
          <p style={{ fontSize: '15px', color: 'rgba(35,56,119,0.55)', textAlign: 'center', padding: '20px 0' }}>
            {filtro === 'Todos' ? 'Ainda não há documentos.' : 'Nenhum documento nesta categoria.'}
          </p>
        ) : (
                ...
                {filtered.map((d) => (
                  <tr key={d.id} style={{ borderBottom: '1px solid rgba(159,142,194,0.12)' }}>
                    ...
                    <td style={{ padding: '14px 8px' }}>
                      <span className={estadoBadgeClass(d.estado)}>{estadoLabel[d.estado]}</span>
                    </td>
                    <td style={{ padding: '14px 8px', fontSize: '14px', color: 'rgba(35,56,119,0.70)' }}>{formatDate(d.created_at)}</td>
                    <td style={{ padding: '14px 8px', textAlign: 'right' }}>
                      {d.ficheiro_url ? (
                        <button onClick={() => handleDownload(d)} className="btn-secondary btn-sm" style={{ padding: '8px 16px' }}>
                          Descarregar
                        </button>
                      ) : (
